...
한 번도 의심한 적 없었죠					
			...
			fw = new FileWriter(folder + "/" + "반딧불이.txt");
...
한 번도 의심한 적 없었죠					
			...
			if(!folder.exists()) folder.mkdirs();
 			...
 			// 문자 기반 파일 출력 스트림 객체 생성
			fw = new FileWriter(folder + "/" + "반딧불이_buffer.txt");
	...
	public void fileInput() {
		 ...
		 * - finally에서 작성하던 close처리 구문을
		 * 	 자동으로 수행하도록 만드는 구문 
		 */
		String path = "io_test/char/반딧불이.txt";
	...
	public void keyboardInput() {
		...
		try(BufferedReader br 
			...
			System.out.printf("문자열 입력1 : ");
			String input1 = br.readLine(); // 입력된 한 줄 읽어오기
			
			System.out.printf("문자열 입력2 : ");
